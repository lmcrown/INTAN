function [OUT]=INTAN_read_Intan_RHD2000_file_multi(fname)
...
% >> plot(t_amplifier, OUT.amplifier_data(1,:))
% 
...
OUT.frequency_parameters = struct( ...
...
OUT.amplifier_channels = struct(channel_struct);
OUT.aux_input_channels = struct(channel_struct);
supply_voltage_channels = struct(channel_struct);
board_adc_channels = struct(channel_struct);
OUT.board_dig_in_channels = struct(channel_struct);
...
if (data_present)
    ...
    OUT.amplifier_data = zeros(num_amplifier_channels, num_amplifier_samples);
    OUT.aux_input_data = zeros(num_aux_input_channels, num_aux_input_samples);
    ...
    OUT.board_dig_in_data = zeros(num_board_dig_in_channels, num_board_dig_in_samples);
    ...
    for i=1:num_data_blocks
        ...
        if (num_amplifier_channels > 0)
            OUT.amplifier_data(:, amplifier_index:(amplifier_index+59)) = fread(fid, [60, num_amplifier_channels], 'uint16')';
        end
        if (num_aux_input_channels > 0)
            OUT.aux_input_data(:, aux_input_index:(aux_input_index+14)) = fread(fid, [15, num_aux_input_channels], 'uint16')';
        end
...
%     OUT.amplifier_data = 0.195 * (OUT.amplifier_data - 32768); % units = microvolts
%     OUT.aux_input_data = 37.4e-6 * OUT.aux_input_data; % units = volts
